	...
	if totalBNOnlineFriends then
      -- iterate through every bnet friend - get their info and add the friend as an interactable line in the tooltip
		...
		if isOnline then
          ...
          local charName = toonName               --gets the friend's character name
          local gameClient = client			           --the application that the friend is online with - can be any game or 'App'/'Mobile'
          local realmName = realmName                  --gets the realm name the friend's char is on
          local faction = factionName                  --gets the friend's currently logged in char's faction
          local zone = ""                        --zone name to be displayed when the friend is playing retail WoW
          local richPresence = ""            --rich presence is used here to determine whether a friend logged into WoW is playing classic
          ...
          if isAFK  then statusIcon = FRIENDS_TEXTURE_AFK end
          -- if the friend is set to 'do not disturb', set the icon left to the friend's name to dnd
          if isDND  then statusIcon = FRIENDS_TEXTURE_DND end
          ...
          if tContains(clientsList, gameClient) or not xb.db.profile.modules.microMenu.hideAppContact then
            ...
            local lineLeft = string.format("|T%s:16|t|cff82c5ff %s|r %s", statusIcon, presenceName, note)
            local lineRight = ''
